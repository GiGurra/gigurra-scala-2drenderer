...
/**
 * MODIFIED BY GiGurra for dcs-remote!
 * 
...
public strictfp final class FastMath {
    ...
    private static final boolean STRICT_MATH = LangUtils.getBooleanProperty("jodk.fastmath.strict", true);

    private static final boolean USE_JDK_MATH = LangUtils.getBooleanProperty("jodk.fastmath.usejdk", false);

    /**
     * Used for both log(double) and log10(double).
     */
    private static final boolean USE_REDEFINED_LOG = LangUtils.getBooleanProperty("jodk.fastmath.fastlog", true);

    private static final boolean USE_REDEFINED_SQRT = LangUtils.getBooleanProperty("jodk.fastmath.fastsqrt", false);
    ...
    public static void sinAndCosDcsRemote(double angle, SinCos out) {
        // Using the same algorithm than sin(double) method, and computing also cosine at the end.
        boolean negateResult;
        if (angle < 0.0) {
            angle = -angle;
        ...
        double indexCos = cosTab[index];
        double result = indexSin + delta * (indexCos + delta * (-indexSin * ONE_DIV_F2 + delta * (-indexCos * ONE_DIV_F3 + delta * indexSin * ONE_DIV_F4)));
        out.s = (float)(negateResult ? -result : result);
        out.c = (float)(indexCos + delta * (-indexSin + delta * (-indexCos * ONE_DIV_F2 + delta * (indexSin * ONE_DIV_F3 + delta * indexCos * ONE_DIV_F4))));
    }
     ...
     */
    public static double remainderTwoPi(double angle) {
        if (USE_JDK_MATH) {
